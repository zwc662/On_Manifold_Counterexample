...
def create_initial_dicts(model, generator, feeder):
    ...
    z, lab, img, pred, loss = generate_images_from_feeder(model, generator, feeder)
...
def noised_images_experiment(model, generator, feeder, epoch, save_path):
    code, images, predictions = create_initial_dicts(model, generator, feeder)
...
def generate_images_from_feeder(model, generator, feeder):
    ...
    z = np.tile(z_tmp, [_len, 1])
    return generate_image_from_code(model, generator, z)

def generate_image_from_code(model, generator, z):
