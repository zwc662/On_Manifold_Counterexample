...
def mnist_train(model, epoch, save_path="./", mode="supervised", input_image=True):
    ...
    save_path_=  save_path + "/" + timestamp + "/"
    ...
    for _e in range(epoch):
        ...
        if _e % 50 == 0:
            ...
            model.saver.save(model.sess, "%s/progress-%i-model.ckpt" % (save_path_, _e))
            np.savez("%s/progress-%i-acc.npz" % (save_path_, _e), loss=np.array(result),
                     learning_rate=model.learning_rate, epoch=epoch, batch_size=model.batch_size,
                     clip=model.max_grad_norm)
    ...
    model.saver.save(model.sess, "%s/model.ckpt" % save_path_)
    np.savez("%s/acc.npz" % save_path_, loss=np.array(result), learning_rate=model.learning_rate, epoch=epoch,
             batch_size=model.batch_size, clip=model.max_grad_norm)
