...
class CNN(object):
    ...
    def _create_network(self):
        ...
        self.loss_ = -self.y * tf.log(self.prediction + 1e-10) - tf.log(1. - self.y + 1e-10) * self.prediction
    ...
    def predict(self, inputs, label):
        ...
        return self.sess.run([self.prediction, tf.reduce_sum(self.loss_, 1)], feed_dict={self.x: inputs, self.y: label})

def prediction_loss(label, prediction, eps = 1e-10):
    return -tf.reduce_sum(label * tf.log(prediction + eps)) 
